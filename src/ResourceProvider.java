...
public class ResourceProvider {
	...
	public void receivePhysician(String senderName) throws InterruptedException {
		isRespondingToRequest.acquire();
		
		this.physicians.release();
		System.out.println("[Resource provider] Received a physician from [" + senderName + "]");
	...
	public boolean giveRoom(String receiverName) throws InterruptedException {
		...
		// Delete a room frome the list of available rooms
		if (rooms.tryAcquire()) {
	...
	public boolean givePhysician(String receiverName) throws InterruptedException {
		isRespondingToRequest.acquire();
		
		// Delete a room frome the list of available rooms
		if (physicians.tryAcquire()) {
			System.out.println("[Resource provider] Sent a physicians to [" + receiverName + "]");
			
			isRespondingToRequest.release();
			return true;
		}
		else {
			//System.out.println("[Resource provider] Couldn't send a physicians to [" + receiverName + "]");
